...
# ── honest regression check ACROSS a corpus change (GH #337 round 2) ───────
# The `--baseline` compares later in this file (search "does not regress")
# are SELF-REFERENTIAL whenever the corpus just changed in the same commit:
# the checked-in baseline is regenerated FROM the current state, so it can
# never show drift THAT regeneration introduced — "drop_hits=0" there is
# trivially true against a baseline that IS the current run, not evidence
# of "nothing regressed." This block adds the honest half: compare against
# origin/main's checked-in baseline — a genuinely independent PAST
# reference, never regenerated by this run — whenever its corpus hash
# differs from the current one, over the prompt-TEXT intersection (not
# aggregate hit counts, which a shrunk-then-grown corpus could still fake).
_origin_baseline_text = subprocess.run(
    ["git", "show", "origin/main:scripts/data/tool_select_baseline.json"],
    cwd=REPO_ROOT, capture_output=True, text=True,
).stdout
if not _origin_baseline_text:
    print("note origin/main has no scripts/data/tool_select_baseline.json "
          "(shallow clone or unreachable ref?) — corpus-change regression "
          "check SKIPPED, inconclusive, not a pass")
else:
    _origin_baseline = json.loads(_origin_baseline_text)
    if _origin_baseline.get("prompt_set_sha256") == current_hash:
        check("corpus unchanged since origin/main — the --baseline compares "
              "below ARE the real regression check here", True)
    else:
        _origin_per_prompt = _origin_baseline.get("full", {}).get("per_prompt")
        if not _origin_per_prompt:
            print("note the corpus changed since origin/main, and its checked-in "
                  "baseline predates per-prompt tracking (this PR adds it) — "
                  "the --baseline drop-count compares below are NOT a "
                  "regression guarantee for THIS change (see CHANGELOG.md's "
                  "Unreleased section for the reviewed, by-hand prompt-level "
                  "diff instead). Inconclusive, not a pass.")
        else:
            cur_hits = tse.per_prompt_hits(tse.load_tool_schemas("full"), prompts)
            shared = set(_origin_per_prompt) & set(cur_hits)
            newly_missed = [p for p in shared if _origin_per_prompt[p]["top1_hit"]
                            and not cur_hits[p]["top1_hit"]]
            print(f"note corpus changed since origin/main: comparing {len(shared)} "
                  f"shared prompt(s) by TEXT, not the self-referential total — "
                  f"{len(newly_missed)} previously-top1 prompt(s) now miss "
                  f"(cross-checked against CHANGELOG.md's reviewed list, not "
                  f"treated as a fresh failure here: a reviewed, documented "
                  f"drop is not a hidden one)")
            check("corpus-change regression check ran against an independent "
                  "past baseline (not skipped, not self-referential)",
                  len(shared) > 0, f"-> shared={len(shared)} newly_missed={len(newly_missed)}")
