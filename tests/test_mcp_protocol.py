...
async def main() -> None:
    async with Client(server.mcp, mode="auto") as c:
        ...
        # ── icons + website_url ──────────────────────────────────────────────
        from codecalc.server import _WEBSITE_URL, GROUP_ICONS, TOOL_GROUPS
        ...
        check("the server carries at least one icon", bool(server.mcp.icons))

        listed = await c.list_tools()
        by_name = {t.name: t for t in listed.tools}
        missing_icons = [n for n, t in by_name.items() if not getattr(t, "icons", None)]
        check("every tool carries at least one icon", not missing_icons, f"-> {missing_icons}")
        ...
        for name, tool in by_name.items():
            ...
            icons = tool.icons or []
            expected_src = GROUP_ICONS[group].src if group in GROUP_ICONS else None
            if expected_src is not None and not any(i.src == expected_src for i in icons):
                wrong_group_icon.append(name)
            for icon in icons:
                if len(icon.src.encode("utf-8")) >= 600:
                    oversize.append((name, len(icon.src)))
                if not icon.src.startswith("data:image/svg+xml;base64,"):
                    bad_svg.append(name)
                    continue
                # A structural check, not a full XML parse: every icon here
                # is generated by this same codebase's own `_svg_icon`, not
                # untrusted input, so a stdlib XML parser (XXE/entity-
                # expansion exposure on arbitrary input) is the wrong tool
                # for what is really "did the string builder produce a
                # single well-formed <svg>...</svg> element with no
                # embedded DOCTYPE/ENTITY it should never contain".
                raw = base64.b64decode(icon.src.split(",", 1)[1]).decode("ascii")
                if not (raw.startswith("<svg ") and raw.endswith("</svg>")
                        and "<!DOCTYPE" not in raw and "<!ENTITY" not in raw
                        and raw.count("<svg") == 1 and raw.count("</svg>") == 1):
                    bad_svg.append(name)
        check("every tool's icon matches its group's GROUP_ICONS entry",
