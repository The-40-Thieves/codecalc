...
> [!NOTE]
> Published as **`codecalc` 0.10.0** on PyPI (`pip install codecalc`) and the
> **`codecalc-exec` 0.10.0** executor on crates.io
...
[![Add to Cursor](https://cursor.com/deeplink/mcp-install-dark.svg)](cursor://anysphere.cursor-deeplink/mcp/install?name=codecalc&config=eyJjb21tYW5kIjoidXZ4IiwiYXJncyI6WyJjb2RlY2FsY1tmdWxsXSJdfQ==)
[![Install in VS Code](https://img.shields.io/badge/VS_Code-Install_codecalc-0098FF?logo=visualstudiocode&logoColor=white)](vscode:mcp/install?%7B%22name%22%3A%22codecalc%22%2C%22type%22%3A%22stdio%22%2C%22command%22%3A%22uvx%22%2C%22args%22%3A%5B%22codecalc%5Bfull%5D%22%5D%7D)
...
names and typed schemas live, and collapsing 57 tools into one dispatcher makes
...
- **Any client** can filter which of the 57 tools it exposes to the model.
...
for the value on each of the 57). Codex CLI's `writes` approval mode
...
Unset or empty registers every group — 57 tools, same as today —
...
70 test files and 19 CI-invoked scripts, **2184 assertions**. "CI-invoked"
