...
> [!NOTE]
> Published as **`codecalc` 0.13.0** on PyPI (`pip install codecalc`) and the
> **`codecalc-exec` 0.13.0** executor on crates.io
...
| `percent_change` | Exact percent change FROM_VALUE→TO_VALUE, relative to `abs(from_value)`; refuses from_value=0 |
...
73 test files and 19 CI-invoked scripts, **2184 assertions**. "CI-invoked"
