...
def benchmark(code: Annotated[str, Field(description="Program that reads integer N from stdin's first line and does work sized by N")],
              ...
              sizes: Annotated[str, Field(description="Comma-separated input sizes to run at, e.g. '100,1000,10000,100000' (this default: 4 sizes 10x apart, decided via curve-fit). 3 is the accepted minimum but can only return noise-floor O(1) or an inconclusive estimate — never a growth class, not even exponential, which needs 2+ agreeing doubling ratios (so 4+ sizes) and cannot decide on one; 4+ sizes are needed to trust the curve fit; 5+ DOUBLING sizes (n,2n,4n,8n,16n) are needed for a robust ratio median")] = "100,1000,10000,100000",
    ...
    sizes (the accepted minimum) can only return noise-floor O(1) or
    "inconclusive" — never a growth class, not even exponential; 4+ sizes are
    needed to trust the curve fit, 5+ doubling sizes for a robust ratio
    median — `estimate_basis` names which estimator decided.
