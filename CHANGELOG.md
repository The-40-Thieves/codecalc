...
- `llms.txt` at the repo root (the [llmstxt.org](https://llmstxt.org/)
  ...
  `llms-full.txt` (generated by `scripts/build_llms_full.py`) inlines the
  full text of everything it links; `scripts/check_llms_txt.py` gates both —
  every linked path must exist, and `llms-full.txt` must match what the
  generator produces from the current `llms.txt`. README.md gained a "Where
  to find codecalc" table (PyPI, crates.io, GitHub Releases, the MCP
  registry, Smithery, Glama, MCPB) and `docs/distribution.md` records the
  submission steps for the two directories that do not list codecalc yet
  (PulseMCP, mcp.so).
