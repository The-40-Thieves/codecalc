...
- **`tools.json`** — the live `tools/list` response of the exact image
  ...
  → `tools/list`), 50 entries, each argument carrying `optional: true` when the tool's input schema does not require it. `CODECALC_TOOLS` is unset in the Dockerfile,
  which registers every tool group (the `full` preset) — so this is the
  actual shipped surface, not `core`. A few of those 50 (`evaluate_expression`,
