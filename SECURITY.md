...
| `no_net` on the native executor | **Linux: enforced in-kernel by a seccomp-bpf filter** — it refuses the `socket(AF_INET/AF_INET6)` SYSCALL, so a `ctypes`/`dlsym` or raw-syscall network call cannot bypass it the way it bypasses a symbol shim (`AF_UNIX` is left alone). A Linux kernel without seccomp falls back to the shim below and discloses it. **macOS: best-effort** — `DYLD_INSERT_LIBRARIES` symbol interposition, which a dynamically-linked `ctypes`/`dlsym` or raw syscall bypasses, and which a SIP-protected/hardened interpreter drops entirely; disclosed in `unenforced`. The strict (gVisor) backend blocks egress at the kernel on any host. (Disclosure E-1; kernel enforcement added since; `docs/security/audit-2026-08-21.md`) |
